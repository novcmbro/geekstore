...
export const ProductsListItem = ({ product }: ProductsListItemProps) => {
  ...
  const { isAdminMenuRoute } = useRoute()
    ...
    <li className="product-list-item" aria-label={product.name}>
      {isAdminMenuRoute ? (
        <div className="button-container">
          ...
          <Link to={`${routesBasePath}/edit-product/${product.id}`} aria-label={`${t("products.edit")} ${product.name}`}>
      ...
      {isAdminMenuRoute ? (
        ...
        <Link to={`${routesBasePath}/product/${product.id}`} className="route-link see-product-link">{t("routes.see-product")}</Link>
      )}
    </li>
  )
}
